    ...
    for key in variables:
        if key in criteria:
            for criterion, patterns in criteria[key].items():
                results.extend(
                    list(set([var for var in df.index if regex.match(patterns, var)]))
                )
...
def select_variables2(variable_strings, criteria, nicknames):
    """Use variable criteria to choose from available variables.
    ...
    nicknames = odg.astype(nicknames, list)
    results = []
    for key in nicknames:
        ...
        if criteria is not None and key in criteria:
            for criterion, patterns in criteria[key].items():
                results.extend(
                    list(set([var for var in variable_strings if regex.match(patterns, var)]))
